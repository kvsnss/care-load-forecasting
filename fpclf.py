...
# --- Input data (demo: synthetic, replace with real CSV upload) ---
uploaded_file = st.file_uploader("Upload present/future data (CSV)", type=["csv"])
if uploaded_file:
    data = pd.read_csv(uploaded_file)
    st.write("Uploaded data preview:", data.head())
    # Example: use first 4 numeric columns as features
    X_input = data.select_dtypes(include=[np.number]).iloc[:horizon, :4].values
else:
